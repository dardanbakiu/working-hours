...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.employee_label = tk.Label(self, text="Employee ID:")
        self.employee_label.grid(row=0, column=0, padx=5, pady=5)
        ...
        self.algorithm_dropdown = ttk.Combobox(self, textvariable=self.algorithm_var, values=[
                                               "Linear Regression", "KNN", "Support Vector Machine"], width=22)
        self.algorithm_dropdown.grid(
            row=1, column=1, padx=5, pady=5)

        # Create the labels and entries for the algorithm parameters
        self.param1_label = tk.Label(self, text="Week day:")
        self.param1_label.grid(row=2, column=0, padx=5, pady=5)
        self.param1_entry = tk.Entry(self)
        self.param1_entry.grid(row=2, column=1, padx=5, pady=5)

        self.param2_label = tk.Label(self, text="Month:")
        self.param2_label.grid(row=3, column=0, padx=5, pady=5)
        self.param2_entry = tk.Entry(self)
        self.param2_entry.grid(row=3, column=1, padx=5, pady=5)
    ...
    def predict(self):
        ...
        if algorithm == "Linear Regression":
            ...
            result = predict_hours_of_work_knn(
                employee_name, param1, param2, n_neighbors=3, weights="uniform")
        else:
...
root = tk.Tk()
root.title("Manual average search")
